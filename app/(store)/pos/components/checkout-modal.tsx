...
export function CheckoutDialog({ disabled, cashier }: Props) {
  ...
  const onSubmit = async (data: CheckoutFormData) => {
    ...
    const sale_data = {
      ...
      sub_total: getTotal(),
      amount_received: data.paymentMethod === 'CASH' ? amountReceived : total,
      customer_change: parseFloat(customerChange),
      discount: discount,
      total: total,
      tax: 0,
    ...
    const result = await createSale(
      ...
      items.map(({ product, quantity }) => {
        ...
        return {
          ...
          selling_price: sellingPrice, // Use the calculated selling price
